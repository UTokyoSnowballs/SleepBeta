    ...
    public class FitbitController : Controller
    {
        ...
        */
        ...
        public async Task<ActionResult> DataSync()
        {
            ...
            //Getting the Fitbit Client Details
            FitbitClient client = GetFitbitClient();
            ...
            string connectionString = "Server=tcp:sleepexplorer-database-server.database.windows.net,1433;Initial Catalog=sleepexplorer-database;Persist Security Info=False;User ID=SleepMakeSensev2db;Password=uQF5mnMVUPwWxNXCtyknztT3nkzma5phrKhVLV46xV76KAQPunjzwBkvRrnV;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;";
            SqlConnection database = new SqlConnection(connectionString);
            string query = "SELECT * FROM Userdatas WHERE ID = " + userName + " ORDER BY DateStamp DESC;";


            //Grabbing Data for the last 40 days
            ...
            if (CNTbodyTemp > 4)
            {
                ...
                pearson = MathNet.Numerics.Statistics.Correlation.Pearson(tempMinutesToFallAsleepBodyTemp, BodyTemp);
                if (Math.Abs(pearson) >= 0.3)
                {
